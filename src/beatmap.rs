...
impl<'a> General<'a> {
    pub fn default_with_context(ctx: &Context) -> Self {
        Self {
            audio_filename: "".into(),
            audio_lead_in: 0,
            audio_hash: "".into(),
...
impl Editor {
    pub fn default_with_context(_ctx: &Context) -> Self {
        Self {
            bookmarks: vec![],
...
impl<'a> Beatmap<'a> {
    pub const DEFAULT_VERSION: i32 = 14;
...
impl<'a> Beatmap<'a> {
    pub fn parse_str(data: &'a str) -> Result<Self, ParseError> {
        ...
        loop {
            let line = data[pos..next_pos].trim_end_matches(['\n', '\r']);
            let mut skip = line.is_empty() || line.starts_with("//");
            let mut eof = false;
            if !skip
                && section != Some(Section::HitObjects)
